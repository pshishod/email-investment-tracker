...
class htmlParser():
    ...
    def parse_txn_details(self, txnHtml):
        ...
        txnObj['refCode'] = navigator.find_all("td")[1].span.contents[0]
        
        navigator = navigator.find_next_sibling("tr")
        txnObj['pmntMthd'] = navigator.find_all("td")[1].contents[0]
        
        navigator = navigator.find_next_sibling("tr")
        dateTime = navigator.find_all("td")[1].contents[0]
        dateTime = dateTime.rsplit(' ', 1)[0]
        dateTimeObj = datetime.strptime(dateTime, "%B %d, %Y %H:%M")
        txnObj['date'] = dateTimeObj.strftime("%B %d, %Y")
        txnObj['time'] = dateTimeObj.strftime("%H:%M")
        navigator = navigator.find_next_sibling("tr")
        ...
        txnObj['exchgRate'] = navigator.find_all("td")[1].contents[0].split()[1].replace('$', '').replace(',', '')
        
        navigator = navigator.find_next_sibling("tr")
        txnObj['amtBght'] = navigator.find_all("td")[1].contents[0].replace(
            '$', '').replace(',', '')
        
        navigator = navigator.find_next_sibling("tr")
        txnObj['fee'] = navigator.find_all("td")[1].contents[0].replace(
            '$', '').replace(',', '')
        
        navigator = navigator.find_next_sibling("tr")
        txnObj['ttlSpnt'] = navigator.find_all("td")[1].contents[0].replace(
            '$', '').replace(',', '')
        return txnObj
